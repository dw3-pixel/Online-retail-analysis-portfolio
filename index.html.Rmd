...
___Installing and loading packages to create a toolkit that will analyze and visualize the data for the analysis.___
...
___Cleaning the data by removing invalid and canceled transactions,then create a new dataframe with columns for "TotalSales", with properly formatted dates, for deeper analysis.___

```{r data import, warning=FALSE, message=FALSE}

# Import and clean the dataset
# Assuming the data had already been imported into the R environment.
...
online_retail_clean <- read_csv("Online Retail.csv", trim_ws = TRUE) %>%
  ...
  mutate(TotalSales = Quantity * UnitPrice,
         InvoiceDate = dmy_hm(InvoiceDate, quiet = TRUE))

# Verification

if(any(is.na(online_retail_clean$InvoiceDate))) {
    # This warning is expected due to some poorly formatted dates in the raw data
    warning("Some InvoiceDates failed to parse with dmy_hm and are NA.")
} # The 'if' ends here

print(paste("Cleaned Rows:", nrow(online_retail_clean)))

```

# --- 2. Exploratory Data Analysis (EDA) ---

___Aggregating the cleaned retail data to calculate and show the monthly revenue and product revenue in new data frames called "monthly_sales" and "top 10 countries" respectively.___

```{r Monthly Revenue Trend}

# Monthly Revenue Trend Calculation

monthly_sales <- online_retail_clean %>%
  mutate(InvoiceMonth = floor_date(InvoiceDate, "month")) %>%
  drop_na(InvoiceMonth) %>% 
  group_by(InvoiceMonth) %>%
  summarise(MonthlyRevenue = sum(TotalSales, na.rm = TRUE)) %>%
  ungroup()


## Monthly Revenue Plotting
# ___Creating a line graph that visualizes the monthly revenue trend from the monthly_sales data frame and making use of formats for clear and easy interpretations.___
...
## Top 10 Countries and Products Calculation
...
knitr::kable(top_10_countries, format = "html")

cat("\n\n### Top 10 Products by Total Revenue\n\n")
knitr::kable(top_products, format = "html")
...
# Top Product by Country Analysis
...
# Calculating a sales summary for each customer, finding out their Customer Lifetime Value (CLV), Order frequency and most recent date of order and storing the result in a new dataframe called "customer_sales_summary".___
...
___Finding every unique product purchased within each order and  store all the orders as a simple list of products,for deeper analysis.___
...
# Generate the interactive widget object using html widget engine
interactive_plot <- plot(top_rules, 
                         method = "graph", 
                         engine = "htmlwidget", 
                         main = "Interactive Association Rules Network (Top 100 Rules)")

# Save the interactive widget as a standalone HTML file for external use

saveWidget(interactive_plot, 
           file="market_basket_network.html", 
           selfcontained = TRUE)
...
___Combining the transformed sales data with the RFM scores and customer segments to create and save a complete dataset, for dashboarding in Tableau.___

```{r dashboard data export}
